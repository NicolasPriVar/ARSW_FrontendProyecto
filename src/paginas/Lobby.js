...
function Lobby() {
    ...
    useEffect(() => {
        const fetchJugadores = async () => {
            try {
                const response = await fetch(`https://mentemaestra-fffra0affsaggzd4.canadacentral-01.azurewebsites.net/api/codigo/jugadores/${codigo}`);
                const data = await response.json();
                setJugadores(data);

                const resEstado = await fetch(`https://mentemaestra-fffra0affsaggzd4.canadacentral-01.azurewebsites.net/api/codigo/estado/${codigo}`);
    ...
    const handleIniciar = async () => {
        try {
            const response = await fetch(`https://mentemaestra-fffra0affsaggzd4.canadacentral-01.azurewebsites.net/api/codigo/iniciar`, {
                method: 'POST',
                headers: {
    ...
    useEffect(() => {
        const verificarEstado = async () => {
            ...
            try {
                const res = await fetch(`https://mentemaestra-fffra0affsaggzd4.canadacentral-01.azurewebsites.net/api/codigo/estado/${codigo}`);
            ...
            <div className="lobby-panel">
                ...
                {rol !== 'admin' && (
                    <BotonSalir onClick={async () => {
                        try {
                            await fetch(`https://mentemaestra-fffra0affsaggzd4.canadacentral-01.azurewebsites.net0/api/codigo/salir`, {
                                method: 'DELETE',
                                headers: {
                                    'Content-Type': 'application/json',
                                },
